...
from src.constants import DATA_DIR, PATENT_DIR, VALID_CODES
...
def get_patent_data(
    ...
    :param graph_name: Name of the graph which is getting scraped.
    :param os_system: The OS on which the code is running. It can be either of these: linux, mac, window.
    :param chrome_driver_path: The path of the chrome driver is located.
    """
    df = pd.read_csv(
        f'{PATENT_DIR}/{graph_name}_chemicals.tsv',
        sep='\t',
        dtype=str,
        usecols=[
            'chembl',
    ...
    if os.path.exists(f'{PATENT_DIR}/{graph_name}_patent_data.tsv'):
        patent_df = pd.read_csv(f'{PATENT_DIR}/{graph_name}_patent_data.tsv', sep='\t')
    else:
    ...
    for chembl_id, surechembl_idx in tqdm(df.values, total=df.shape[0]):
        ...
        if cache_count == 5:  # in case of internet issues
            ...
            patent_df.to_csv(f'{PATENT_DIR}/{graph_name}_patent_data.tsv', sep='\t', index=False)
            cache_count = 0
