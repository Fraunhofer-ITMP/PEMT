...
from src.constants import MAPPER_DIR
from src.utils import hgnc_to_chembl, uniprot_to_chembl
...
def enrich_chemicals(
    ...
    :param file_separator: The separator used within the file. This can be 'csv', 'tsv', or 'semicolon'.  By default,
    ...
    chembl_mapper = pd.read_csv(
        f'{MAPPER_DIR}/chembl_uniprot_mapping.tsv',
        dtype=str,
        skiprows=1,
        sep='\t',
    ...
    # Extract the gene
    if file_separator != ',':
        if file_separator == 'semicolon':
            file_separator = ';'
        else:
            file_separator = '\t'

    if gene_file_path and file_separator and is_uniprot:
        proteins = set(pd.read_csv(
            gene_file_path,
            sep=file_separator
        )['uniprot'].tolist())
    elif gene_file_path and file_separator and not is_uniprot:
        proteins = set(pd.read_csv(
            gene_file_path,
            sep=file_separator
        )['symbol'].tolist())
    ...
    for hgnc_symbol in tqdm(proteins, desc='Extracting chemicals for targets'):
        if hgnc_symbol in gene_chemical_dict:
            continue
        ...
        gene_chemical_dict[hgnc_symbol] = chemical_list
